...
默认使用 CPU、固定随机种子，先训练一维 LQR 2000 次，再训练摆的四阶段、每阶段 3000 次。每次运行新建结果目录，保留网络、数据、随机状态、全部 loss、测试残差、闭环轨迹、源文件哈希和图。`'smoke'` 仅用 10 次更新检查执行路径；`'quartic'` 为可选的**修正代价**算例配置，本轮没有执行该训练。
