...
- A converted MATLAB weight file. The loader expects `W1`–`W5`, `b1`–`b5`, `checkpoint_epoch` and independent reference value/gradient arrays; see [`load_author_boat.m`](load_author_boat.m) for the exact fields. [`SOURCE.json`](SOURCE.json) records the historical checkpoint and normalization. Weights and conversion tools are not bundled, and a raw PyTorch `.pth` file cannot be passed directly to the loader.
...
The [September 7 record](../../evidence/v0.3/safe-pinn/RESULTS.md) reports the following fixed-input evaluations with the current cost integrator:
...
The recorded reduced/author audits had 15/300 and 2/300 joint violations, with respective 95% one-sided upper bounds 7.5949% and 2.0836%. These concern the selected set and numerical rollout labels. See the [equation notes](../../docs/fulltext/safe_pinn_icml2025.md) for the sampling and step-refinement checks.
