...
The safe-PINN author weights are a separate asset, not a fresh training run. Native [net,parity]=load_author_boat(weightFile) loads the converted weights and checks golden values/gradients. result=evaluate_boat_model(net,outputDirectory,comparisonFile) requires a previously saved demo_safe_pinn result and reuses its held-out inputs/initial states. Obtain and convert the author weights separately, then pass their explicit local path; no checkpoint is bundled. Use `fullfile(runDir,'result.mat')` from a new `demo_safe_pinn` run as the comparison file. Loading and evaluating this network requires Deep Learning Toolbox. The default safe selection in demo_reproductions still calls the reduced-training demo, not the author checkpoint.
