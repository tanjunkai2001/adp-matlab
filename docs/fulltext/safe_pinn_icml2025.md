...
**公开代码对照。** 固定提交的训练调用链实际使用 `V-max(g,phi-z)`，而非论文(6)中的 `V-g`；标准批量验证还会裁剪状态和预算，公开安全校准统计的是碰撞事件。因而本平台的未裁剪动力学与联合成本/安全审计不能直接套用作者演示阈值-0.02。另有一处终端成本赋值顺序错误仅确认出现在演示脚本，批量性能脚本的该顺序正确。精确位置、原因和适用范围见[作者源码审计](safe_pinn_author_code_audit.md)。这些源码事实不能证明发布检查点的完整训练过程。
