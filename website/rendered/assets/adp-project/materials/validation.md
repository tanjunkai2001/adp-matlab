...
[Summary](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/matlab-tests.json) · [Per-test CSV](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/matlab-tests.csv) · [81 source hashes](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/matlab-source-hashes.json)
...
One PINN replay test executes two single Adam updates to check determinism. They do not retrain the historical paper experiments. Only the mean-field demo and its test file changed among the 81 MATLAB files. The [original v0.5.0 test record](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/v0.5.0/matlab-tests.json) and [source hashes](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/v0.5.0/matlab-source-hashes.json) are preserved.

The new regression exercises a real four-window rank-deficient primary fit: its configuration, raw observations and statistics survive the original `mf:RankDeficient` error and reproduce that rejection. A separate reduced success run matched the preceding source exactly for the saved inputs, fitted iterations, mean trajectory and evaluation, excluding elapsed time. See the [configuration and comparison record](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/meanfield-failure-replay.json). These are maintenance checks; the historical paper experiments were not rerun.

## Numerical reproduction scope

| Method | Recorded result and unresolved scope |
|---|---|
| Baseline | Analytic LQR gain/value comparison and Bellman checks. Figures retain the recorded September 7 experiments. A fresh September 8 quickstart independently returns the same gain; see its [metrics](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/quickstart-baseline.json). |
...
See the [method cards](https://github.com/tanjunkai2001/adp-matlab/tree/v0.5.1/docs/fulltext) for source versions, equations, configurations and each result’s scope. Historical records are described in [ARTIFACTS](https://github.com/tanjunkai2001/adp-matlab/blob/main/docs/ARTIFACTS.md).

## Publication infrastructure

The [first GitHub Actions run](https://github.com/tanjunkai2001/adp-matlab/actions/runs/34177624955) passed both repository checks and **102/102 MATLAB tests** on Ubuntu 24.04 with MATLAB R2025b Update 6, Control System Toolbox and Deep Learning Toolbox. It tested commit `b5f104d86662cd9c80c7751f43ff46c5ef9e9a2a`. The [cloud run record](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/github-actions-first-run.json) and [per-test CSV](https://github.com/tanjunkai2001/adp-matlab/blob/v0.5.1/docs/validation/github-actions-tests.csv) preserve that result; subsequent runs appear in [Actions](https://github.com/tanjunkai2001/adp-matlab/actions/workflows/checks.yml). Full paper experiments on another machine remain a separate check.
